...
void SD_Input_Data(char * num,uint8 f_name[],uint8 path_name[]) //向sd卡写数据
  {
    FIL     fdst;   //文件
    FATFS   fs;     //文件系统
    uint32 size,sizetmp;
    ...
    if(f_mkdir(f_name))
    {
    res = f_open(&fdst, path_name, FA_OPEN_ALWAYS | FA_WRITE | FA_READ);  //打开文件，如果没有就创建，带读写打开
...
void SD_Read_Data(uint8 f_name[],uint8 path_name[],uint8 Id,uint8 num)           //向sd卡读数据
    ...
    f_lseek(&fdst,Id-1);                      //把指针指向文件指定位置
    
    f_read (&fdst, buff, num, &sizetmp);   //读取
