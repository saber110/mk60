...
void uart_getstr (UARTn_e uratn, char *data,int len)
{
    //等待接收到数据（使能了FIFO后，以下两种方法都可行，都是一样的意思）
    //while( !UART_RCFIFO_REG(UARTN[uratn]) );                                //等待接收到数据数量非0
    //while( (UART_SFIFO_REG(UARTN[uratn]) & UART_SFIFO_RXEMPT_MASK)) ;     //等待接收缓冲区/FIFO 非空的
 ...
 *  @param      str      字符串地址_页面名
*/
void UART_Page(char *str)
{
  
    uart_putstr(UART4,"page ");
 ...
 *  @param      num     数据
*/
void UART_Input_Num(char *str,uint8 num)
{
    uart_putstr(UART4,str);
    printf(".val=%d",num);
    UART_Send_End();
 ...
 *  @param      len       一次画线的长度
*/
void UART_DrawLine(char* buff,uint8 len)
{
  for(uint8 i=0;i<len;i++)
  {
  printf("add 1,0,%d",buff[i]);
  UART_Send_End();
  DELAY_MS(10);
  }
}
 ...
 *  @brief      在任意位置画线
 *  @param      buff      数组地址_数据
 *  @param      str       字符串地址_曲线颜色
 *  @param      len       一次画线的长度
*/
void UART_DrawLine_Arb(char* buff,char* str,uint8 len)
{
  for(uint8 i=0;i<len;i++)
  {
  printf("line %d,%d,%d,%d,",i,buff[i],i,buff[i]);
  uart_putstr(UART4,str);
  UART_Send_End();
  DELAY_MS(10);
  }
